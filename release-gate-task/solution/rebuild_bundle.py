...
def main():
  ...
  # Reference wheel (cp312 first in manifest): folded certifi, correct dist-info path
...
"""
  sha312, sz312 = write_wheel(
    ...
    wheel_bytes([("widgetlib-2.0.09", meta312)], "cp312-cp312-manylinux_2_28_x86_64.manylinux_2_28_x86_64"),
  )

  # Universal: decoy dist-info (no certifi) + correct post1 dist-info
...
"""
  sha310, sz310 = write_wheel(
    ...
    wheel_bytes([("widgetlib-2.0.9", meta310)], "cp310-cp310-manylinux_2_17_x86_64"),
  )
...
Version: 2.0.8
"""
    info = tarfile.TarInfo("widgetlib-2.0.9/PKG-INFO")
    data = pkg.encode()
    info.size = len(data)
  ...
  sdist_data = sdist_buf.getvalue()
  (ART / "widgetlib-2.0.9.tar.gz").write_bytes(sdist_data)
